...
ghdl synth --std=08 -fsynopsys --latches --work=processor --workdir=build -Pbuild --out=verilog rom_wb > "$PATH_TO_VERILOG_FOLDER"/rom_wb.v
ghdl synth --std=08 -fsynopsys --latches --work=processor --workdir=build -Pbuild --out=verilog ram_wb > "$PATH_TO_VERILOG_FOLDER"/ram_wb.v
